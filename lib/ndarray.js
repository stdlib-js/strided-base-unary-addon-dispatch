...
function dispatch( addon, fallback ) {
	if ( !isFunction( addon ) ) {
		throw new TypeError( format( '1Nf3c', addon ) );
	}
	if ( !isFunction( fallback ) ) {
		throw new TypeError( format( '1Nf2H', fallback ) );
	}
	return dispatcher;
	...
	*/
	function dispatcher( N, dtypeX, x, strideX, offsetX, dtypeY, y, strideY, offsetY ) { // eslint-disable-line max-len
		...
		if ( dtypeX === null || dtypeY === null ) {
			throw new TypeError( format('1Nf1L') );
		}
		if ( !isNonNegativeInteger( offsetX ) ) {
			throw new TypeError( format( '1NfAG', offsetX ) );
		}
		if ( !isNonNegativeInteger( offsetY ) ) {
			throw new TypeError( format( '1NfAE', offsetY ) );
		}
